...
## Продвинутая инструкция по развертыванию service-desk-bot
...
```sql
CREATE USER _user PASSWORD '<GENERATE>';
CREATE DATABASE _db;
GRANT ALL PRIVILEGES ON DATABASE _db 
  TO _user;
```
...
```bash
git clone <THIS_REPOSITORY> /opt/express/bots/service-desk-bot
cd /opt/express/bots/service-desk-bot
```

3. Соберите образ:

```bash
docker build -t service-desk-bot .
```
...
4. В директории `/opt/express/bots/service-desk-bot` создайте файл
